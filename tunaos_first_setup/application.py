...
import grp
_ = __builtins__["_"]
from tunaos_first_setup.window import VanillaWindow
...
class FirstSetupApplication(Adw.Application):
    ...
    def __init__(self, moduledir: str, *args, **kwargs):
        ...
        super().__init__(
            *args,
            application_id="org.tunaos.FirstSetup",
    ...
    def do_activate(self):
        ...
        if  self.install_mode:
            ...
            configure_system_mode = False
            # if the running user is in the tunaos-first-setup group, enable configure system mode
            # this is used to run first-setup automatically on first boot
            # then disabled later so it won't run again
            if "tunaos-first-setup" in all_groups and os.getlogin() in grp.getgrnam("tunaos-first-setup").gr_mem:
        ...
        provider.load_from_resource("/org/tunaos/FirstSetup/style.css")
