...
def is_live_session() -> bool:
    ...
    Returns:
        bool: True if kernel command line contains 'tunaos.live=1', False otherwise.
    """
    try:
        with open("/proc/cmdline", "r") as f:
            cmdline = f.read()
        return "tunaos.live=1" in cmdline
    except Exception:
        return False

# ── Variant detection ──────────────────────────────────────────────────────────

# Variant → emoji mapping for TunaOS variants
_VARIANT_EMOJI = {
    "yellowfin": "\U0001F420",   # 🐠
    "albacore":  "\U0001F41F",   # 🐟
    "skipjack":  "\U0001F363",   # 🍣
...
def get_variant_info() -> dict:
    """Read variant info from /usr/lib/os-release.

    Returns dict with keys: variant_id, pretty_name, emoji
    """
    variant_id = None
    pretty_name = "TunaOS"
    ...
    emoji = _VARIANT_EMOJI.get(variant_id, "\U0001F41F")  # default to 🐟

    return {
        "variant_id": variant_id or "tunaos",
