...
@Gtk.Template(resource_path="/org/tunaos/FirstSetup/gtk/install-progress.ui")
class VanillaInstallProgress(Adw.Bin):
    ...
    def __init__(self, window, **kwargs):
        ...
        try:
            data = Gio.resources_lookup_data("/org/tunaos/FirstSetup/gtk/install-progress.ui", Gio.ResourceLookupFlags.NONE)
    ...
    def __handle_json_line(self, line: str):
        """Parse and handle a single JSON line from install-to-disk output."""
        try:
        ...
        if event_type == "step":
            ...
            message = event.get("message", "")
            print(f"[WARNING] install: {message}")
            ...
            print(f"[ERROR] install: {message} - {error_detail}")
            error_text = message
            if error_detail:
    ...
    def __run_install(self):
        ...
        hostname = getattr(self.__window, "install_hostname", None) or ""
        print("[DEBUG] __run_install params:", image, device, fs, "fde_enabled:", fde_enabled, "tpm_enabled:", tpm_enabled, "target_imgref:", target_imgref, "hostname:", hostname)
        ...
        # Build script arguments: image, device, filesystem, fde, passphrase, tpm2, target_imgref, hostname
        script_args = [
            image,
            device,
            fs,
            ...
            hostname,        # 8th arg: desired hostname for installed system
        ]
